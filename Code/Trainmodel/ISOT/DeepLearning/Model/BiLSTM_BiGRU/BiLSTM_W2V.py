...
# Huấn luyện Word2Vec
embedding_dim = 100
w2v_model = Word2Vec(sentences=X_train_tokens, vector_size=embedding_dim, window=5, min_count=1, workers=4)
...
# Tạo Embedding Matrix từ Word2Vec
...
# Build model với Word2Vec
model = tf.keras.Sequential([
    tf.keras.layers.Embedding(input_dim=vocab_size, output_dim=embedding_dim, 
                              weights=[embedding_matrix], trainable=False),  # Dùng Word2Vec
...
history_log_filename = f"BiLSTM_W2V_training_log_{timestamp}.csv"
...
loss_plot_filename = f"BiLSTM_W2V_training_loss_{timestamp}.png"
...
plt.legend(prop={'size': 15})
plt.ylim((0.5, 1))
accuracy_plot_filename = f"BiLSTM_W2V_training_accuracy_{timestamp}.png"
...
metrics_df.to_csv(f"BiLSTM_W2V_evaluation_metrics_{timestamp}.csv", index=False)

# Classification Report
report_df = pd.DataFrame(classification_report(y_test, y_pred, output_dict=True)).transpose()
report_df.to_csv(f"BiLSTM_W2V_classification_report_{timestamp}.csv", index=True)

# Confusion Matrix
matrix = confusion_matrix(y_test, y_pred)
matrix_df = pd.DataFrame(matrix, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"])
matrix_filename = f"BiLSTM_W2V_confusion_matrix_{timestamp}.csv"
...
confusion_matrix_plot_filename = f"BiLSTM_W2V_confusion_matrix_{timestamp}.png"

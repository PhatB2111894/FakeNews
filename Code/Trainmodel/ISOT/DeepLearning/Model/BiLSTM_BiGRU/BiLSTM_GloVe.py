...
#fakeorreal
#faknewdetection
#isot-fakenew-dataset

df1 = pd.read_csv("../input/isot-fakenew-dataset/train.csv", delimiter=";", names=["text", "label"], skiprows=1)
X_train = df1["text"]
y_train = df1["label"]

df2 = pd.read_csv("../input/isot-fakenew-dataset/test.csv", delimiter=";", names=["text", "label"], skiprows=1)
X_test = df2["text"]
y_test = df2["label"]
# # Load dataset
...
# Huấn luyện GloVe
embedding_dim = 100
glove_path = '../working/glove_data/glove.6B.100d.txt'
...
# Build model với GloVe
model = tf.keras.Sequential([
    tf.keras.layers.Embedding(input_dim=vocab_size, output_dim=embedding_dim, 
                              weights=[embedding_matrix], trainable=True),  # Dùng GloVe
    tf.keras.layers.SpatialDropout1D(0.3),  # Tăng khả năng tổng quát hóa
    tf.keras.layers.Bidirectional(tf.keras.layers.GRU(128, return_sequences=True)),
    tf.keras.layers.Bidirectional(tf.keras.layers.GRU(64, dropout=0.3)),
...
# Compile model
model.compile(loss='binary_crossentropy', optimizer=tf.keras.optimizers.Adamax(1e-5), metrics=['accuracy'])

# Train model
early_stop = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=5, restore_best_weights=True)
history = model.fit(X_train, y_train, epochs=50, batch_size=128, shuffle=True, callbacks=[early_stop])
...
history_log_filename = f"BiGRU_GloVe_isot-fakenew-dataset_training_log_{timestamp}.csv"
...
loss_plot_filename = f"BiGRU_GloVe_isot-fakenew-dataset_training_loss_{timestamp}.png"
...
plt.legend(prop={'size': 15})
plt.ylim((0.5, 1))
accuracy_plot_filename = f"BiGRU_GloVe_isot-fakenew-dataset_training_accuracy_{timestamp}.png"
...
metrics_df.to_csv(f"BiGRU_GloVe_isot-fakenew-dataset_evaluation_metrics_{timestamp}.csv", index=False)

# Classification Report
report_df = pd.DataFrame(classification_report(y_test, y_pred, output_dict=True)).transpose()
report_df.to_csv(f"BiGRU_GloVe_isot-fakenew-dataset_classification_report_{timestamp}.csv", index=True)

# Confusion Matrix
matrix = confusion_matrix(y_test, y_pred)
matrix_df = pd.DataFrame(matrix, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"])
matrix_filename = f"BiGRU_GloVe_isot-fakenew-dataset_confusion_matrix_{timestamp}.csv"
...
confusion_matrix_plot_filename = f"BiGRU_GloVe_isot-fakenew-dataset_confusion_matrix_{timestamp}.png"

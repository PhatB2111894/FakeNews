#BoW + MLP (Multi-Layer Perceptron)
import pandas as pd
import re
import pickle
...
# Save tokenizer
with open(f"MLP_BoW_vectorizer.pkl", "wb") as f:
...
history_df.to_csv(f"MLP_BoW_training_log_{timestamp}.csv", index=False)
...
plt.legend()
plt.savefig(f"MLP_BoW_training_loss_{timestamp}.png")
plt.show()
...
plt.legend()
plt.savefig(f"MLP_BoW_training_accuracy_{timestamp}.png")
plt.show()
...
metrics_df.to_csv(f"MLP_BoW_evaluation_metrics_{timestamp}.csv", index=False)

# Classification Report
report_df = pd.DataFrame(classification_report(y_test, y_pred, output_dict=True)).transpose()
report_df.to_csv(f"MLP_BoW_classification_report_{timestamp}.csv", index=True)

# Confusion Matrix
matrix = confusion_matrix(y_test, pred)
matrix_df = pd.DataFrame(matrix, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"])
matrix_df.to_csv(f"MLP_BoW_confusion_matrix_{timestamp}.csv", index=True)
...
plt.savefig(f"MLP_BoW_confusion_matrix_{timestamp}.png")
plt.show()

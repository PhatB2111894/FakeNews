#LSTM Embedding Layer & Tokenizer
import pandas as pd
import re
import pickle
...
# Build model (LSTM thay vì BiLSTM)
model = tf.keras.Sequential([
    tf.keras.layers.Embedding(vocab_size, embedding_dim),
    tf.keras.layers.LSTM(embedding_dim, return_sequences=True),  # LSTM thường
    tf.keras.layers.LSTM(16),  # LSTM thường
...
history_log_filename = f"LSTM_Tokenizer_training_log_{timestamp}.csv"
...
loss_plot_filename = f"LSTM_Tokenizer_training_loss_{timestamp}.png"
...
plt.legend(prop={'size': 15})
plt.ylim((0.5, 1))
accuracy_plot_filename = f"LSTM_Tokenizer_training_accuracy_{timestamp}.png"
...
metrics_df.to_csv(f"LSTM_Tokenizer__evaluation_metrics_{timestamp}.csv", index=False)

# Classification Report
report_df = pd.DataFrame(classification_report(y_test, y_pred, output_dict=True)).transpose()
report_df.to_csv(f"LSTM_Tokenizer__classification_report_{timestamp}.csv", index=True)

# Confusion Matrix
matrix = confusion_matrix(y_test, y_pred)
matrix_df = pd.DataFrame(matrix, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"])
matrix_filename = f"LSTM_Tokenizer_confusion_matrix_{timestamp}.csv"
...
confusion_matrix_plot_filename = f"LSTM_Tokenizer_confusion_matrix_{timestamp}.png"

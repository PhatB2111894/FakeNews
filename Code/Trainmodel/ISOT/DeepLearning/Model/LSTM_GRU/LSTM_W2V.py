...
# Tokenization
vocab_size = 10000
embedding_dim = 100  # Tăng lên để phù hợp với Word2Vec
max_length = 256
...
# Train Word2Vec
# Lấy danh sách câu gốc trước khi tokenization
sentences = [text.split() for text in df['title_text']]
...
history_log_filename = f"LSTM_Word2Vec_training_log_{timestamp}.csv"
...
loss_plot_filename = f"LSTM_Word2Vec_training_loss_{timestamp}.png"
...
plt.legend(prop={'size': 15})
plt.ylim((0.5, 1))
accuracy_plot_filename = f"LSTM_Word2Vec_training_accuracy_{timestamp}.png"
...
metrics_df.to_csv(f"LSTM_Word2Vec_evaluation_metrics_{timestamp}.csv", index=False)

# Classification Report
report_df = pd.DataFrame(classification_report(y_test, y_pred, output_dict=True)).transpose()
report_df.to_csv(f"LSTM_Word2Vec_classification_report_{timestamp}.csv", index=True)

# Confusion Matrix
matrix = confusion_matrix(y_test, y_pred)
matrix_df = pd.DataFrame(matrix, index=["Actual 0", "Actual 1"], columns=["Predicted 0", "Predicted 1"])
matrix_filename = f"LSTM_Word2Vec_confusion_matrix_{timestamp}.csv"
...
confusion_matrix_plot_filename = f"LSTM_Word2Vec_confusion_matrix_{timestamp}.png"
